...
4. **run-once падает из-за сети/GitHub timeout**
   - проверьте интернет/прокси/VPN
   - уменьшите `GITHUB_CODE_PAGES` и `GITHUB_REPO_PAGES`
   - попробуйте снова: ошибки сети теперь пропускаются, цикл не должен падать полностью

5. **В боте ошибка `Message is not modified`**
   - это не критично: теперь обработано безопасно (показывается "Данные не изменились")
